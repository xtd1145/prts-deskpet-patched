...
10. **快捷启动** — 托盘新增「快捷启动」子菜单（点击即启动）+「管理快捷启动…」窗口（`src/main/quicklaunch.js`、`src/renderer/quicklaunch.html/.js`）：自动检测 **Steam / SakuraCat / 哔哩哔哩直播姬** 的安装路径，也可自行增删任意程序（名称/路径/可选启动参数）。**自己添加方法**：① 管理窗口「浏览…」选择程序 exe 后点「添加」；② 直接编辑数据目录 `settings.json` 的 `quickLaunch` 数组，例如
    ```json
    "quickLaunch": [
      { "id": "steam", "name": "Steam", "path": "D:\\steam\\steam.exe", "args": "" },
      { "id": "minecraft", "name": "我的世界", "path": "C:\\Program Files\\MC\\mc.exe", "args": "--offline" }
    ]
    ```
